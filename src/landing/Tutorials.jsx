    ...
    pages: [
      ...
      },
      {
        ...
        steps: [
          ...
          { heading: "Any equation, any image", body: "None of this is tied to the gyroid or to the plasma. Here the Schwarz P surface, cos x + cos y + cos z = 0, is clad in brick — albedo and a matching normal map, both built-ins — and lit by a swinging lamp, so the level set reads as a carved stone lattice rather than flowing colour. Same technique throughout: project the image from three planes, blend by the normal, bump and light at the hit. Swap the equation, swap the texture, and the pipeline just follows.", kind: "tut-tex-schwarz" },
        ],
      },
    ],
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        steps: [
          { heading: "Lit shading with true normals", body: "Shading is a plot parameter, so it lives on the transformer that renders a map, not on the map itself. Render a map z = f(x,y) through a transformer in graph mode and set its Shading to Lit: now it is lit per pixel with a key light and a specular highlight. The normal is the EXACT analytic normal, taken from the symbolic derivative of the mapped output, so the highlight tracks the real surface rather than the mesh facets — it stays smooth at low grid resolution and as the surface animates. (When a derivative can't be taken, it falls back to screen-space normals.) This is something a mesh engine can't do, because it threw the function away.", kind: "lit-ripple" },
          { heading: "Colour from an expression", body: "A material channel is just another expression evaluated per fragment over the domain (x, y). In ramp mode a scalar is mapped across two colours; here sign(sin(2.4x))·sign(sin(2.4y)) draws a checkerboard. Because it is evaluated per pixel rather than interpolated between vertices, the squares stay crisp no matter how coarse the mesh.", kind: "mat-checker" },
          { heading: "A full RGB colour field", body: "When a single gradient isn't enough, switch the colour to RGB and give three independent expressions for red, green, and blue. Here R = ½+½sin(x), G = ½+½cos(y), B = ½+½sin(x+y) paint a smooth colour field across the surface — any colour, anywhere, as a function of the domain.", kind: "mat-rgb" },
          { heading: "Emission and motion", body: "Emission is added after the lighting, so it self-illuminates regardless of the light. Here glow bands pow(max(0, sin(2.5(x+y) − t)), 3) sweep across a dark dome, driven by an animator t wired into the map that the material reads as a live value. Colour, specular, and emission are all the same operation: evaluate an expression per fragment and feed it into the shading.", kind: "mat-glow" },
          { heading: "A texture as the colour", body: "A texture is just another colour source. Wire a Texture node (here the default Dedekind tile) into the transformer and set its Material colour to Texture: the image is sampled at the surface's UV — which is simply its grid coordinates, already there because the surface is parameterized, no unwrapping needed. A Video node works the same way, uploading its current frame each tick.", kind: "tex-surface" },
          { heading: "Normal maps for surface detail", body: "A normal map adds fine surface detail without adding a single polygon. Wire a second Texture node, set its role to Normal map, and the lit normal is perturbed per pixel by the texture — here a flat dome reads as a field of embossed pyramids. The tangent frame is reconstructed in screen space, so it works on any surface with no precomputed tangents; the normal-map strength on the surface scales the bumps. It shares the same UV transform as the colour texture.", kind: "normal-map" },
          { heading: "Lighting the scene", body: "A light is a scene entity, not a material setting: add a Light node and wire it into the camera, and every lit surface in that view is shaded by it. A directional light is a sun (only its direction matters); a point light sits at a position with inverse-square falloff. Each has a colour and intensity, and every field is an expression — so wiring an animator in moves or dims the light live. Here a warm point light orbits the surface while a cool directional light fills from the side. With no light wired, a default key light is used, so existing scenes are unchanged.", kind: "lights" },
          { heading: "Putting it together: the brick sphere", body: "Every piece at once. A parametric sphere carries a brick albedo texture and a matching brick normal map (both built-ins, referenced by name so the project stays shareable), lit by an orbiting point light and a cool directional fill. The geometry is a perfectly smooth sphere — every brick edge and mortar groove is the normal map perturbing the lit normal, sampled at the sphere's own (u, v). The albedo and the normal map share one layout, so colour and relief line up. This is the canonical shading test, built entirely from nodes.", kind: "brick-sphere" },
        ],
      },
      {
    ...
    pages: [
      ...
      },
      {
        ...
        steps: [
          ...
          { heading: "A minimal surface", body: "Curvature, not just connectivity, gives surfaces character. A minimal surface is one that locally minimizes area, balancing its bending so the mean curvature is zero everywhere — it saddles equally in both directions at every point. The Schwarz P surface, cos x + cos y + cos z = 0, is the classic triply-periodic example: it carves space into two identical interpenetrating networks of channels meeting at right angles, the 'plumber's nightmare'. Boundaryless and of unbounded genus, ray-marched straight from its equation and coloured by orientation.", kind: "tut-schwarz" },
          { heading: "Another minimal surface, lit and textured", body: "The gyroid is a different triply-periodic minimal surface — same minimal, zero-mean-curvature property as the Schwarz P, but a chiral, interwoven topology all its own. Here it is the renderer flexing: ray-marched from one equation, triplanar-textured, bump-lit, reacting to moving lights, and morphing, all at once. Topology you can reach out and texture.", kind: "implicit-tex" },
        ],
      },
    ],
  },
