...
const SECTIONS = [
  {
    ...
    pages: [
      ...
      },
      {
        ...
        summary: "Not everything is a continuous surface. Raw geometry plots discrete sets of points: explicit lists, formulas over an index, and recurrences.",
        steps: [
  ...
  {
    id: "rendering",
    title: "Rendering: shading and materials",
    blurb: "How the renderer shades what the graph produces: lit surfaces with normals taken from the function, and material channels — colour, specular, glow — set by per-pixel expressions.",
    pages: [
      {
    ...
    pages: [
      ...
      },
      {
        ...
        steps: [
          ...
          { heading: "Move the roots yourself", body: "The zeros need not be roots of unity. Here f(z) = (z − r₁)(z − r₂)(z − r₃) with the three roots on sliders, started at the cube roots of unity, which is f(z) = z³ − 1. Drag a root and its black zero slides with it, the colours still winding once around each since every factor is simple. The brightness is the product of the three distances to the roots and the hue is the sum of the three angles, so the whole picture is assembled one root at a time. Pull two roots close and the hue between them races; push one far off and its pull fades.", kind: "dc-movable" },
          { heading: "From coefficients instead", body: "Dragging the roots builds the polynomial from its factors. Turn it around: set the coefficients and let the roots fall where they must. Here f(z) = a z³ + b z² + c z + d with a, b, c, d on four complex sliders, and the three solutions appear as the three glow centres of the colouring. Move a coefficient and all three roots shift at once, since each depends on the whole polynomial, not on any one factor. Start at a = 1, c = −1, the rest zero, giving z³ − z with roots at 0 and ±1; nudge d to lift the curve off zero and watch the triple split and migrate. Unlike the quintic above, a cubic does have a formula for its roots in the coefficients, but the picture finds them whether or not you care to write it.", kind: "complex-cubic" },
          { heading: "One degree higher", body: "The same machine runs a degree up: f(z) = a z⁴ + b z³ + c z² + d z + k on five complex sliders, now with four glow centres to track. Start at a = 1, c = −1, giving z⁴ − z² = z²(z − 1)(z + 1), a double root at the origin and simple roots at ±1; the double root shows as a brighter, faster-winding centre, the hue running around it twice. Nudge k and the double root splits into two that drift apart. The quartic is the last degree that still has a formula in radicals, Ferrari's, more involved than the cubic's but finite. Past here, at degree five, that thread runs out, which is exactly where the next page begins.", kind: "complex-quartic" },
        ],
      },
      {
        slug: "littlewood-polynomials",
        title: "Roots of a whole family",
        summary: "Stop plotting one polynomial and plot every polynomial of a kind at once. The roots of all the ±1 polynomials together draw a fractal ring, first as a cloud of points and then as a live implicit field.",
        steps: [
          { heading: "Every sign of every coefficient", body: "Instead of one polynomial, take all of them with coefficients restricted to +1 and -1, the Littlewood polynomials, and plot every root at once. Shown here for all such polynomials of degree up to ten, each root coloured by the degree it came from. As the degree climbs the roots fill an annulus around the unit circle and resolve into a fractal, with conspicuous holes punched out near the simple roots of unity (1, -1, the cube and fourth roots) where no ±1 polynomial of low degree can vanish. The roots of one polynomial were a handful of points; the roots of a whole family draw a shape. Each root is found numerically, since beyond degree four there is no formula.", kind: "littlewood" },
          { heading: "The same roots as a live field", body: "The cloud above is a precomputed list of points. The same roots can instead be drawn implicitly: take F(z) = the smallest value of |c0 + c1 z + ... + cd z^d| over all the coefficient choices, which is exactly zero where some such polynomial vanishes. That makes F = 0 the implicit equation of the root set, and colouring brightness by a glow that lights up where F approaches zero draws the roots with no list at all, evaluated per pixel so it pans and zooms live. Each coefficient picks between two values you set, va and vb: leave them at 1 and -1 for the Littlewood polynomials, set them to 1 and 0 for the Newman polynomials, or try any pair. The max-degree and glow sliders are live too. The price is that F is a minimum over 2^d coefficient choices, so the expression doubles with every added degree; this runs to degree five, enough to show the ring and its gaps filling in. The full fractal needs a still higher degree, too many choices for one formula, so the dense view stays a computed cloud.", kind: "littlewood-field" },
        ],
      },
    ],
  },
    ...
    pages: [
      {
        ...
        steps: [
          { heading: "The elements on a circle", body: "Number N points 0 to N − 1 evenly around a circle. They are the elements of the cyclic group of order N, the whole numbers under addition mod N, with 0 at the top and each point one step further on than the last. Adding 1 moves to the next point; adding K jumps K places, wrapping back to 0 after N. Drag N to set how many elements the group has. The chords that join them come next.", kind: "cyclic-points" },
          { heading: "One step that reaches everything", body: "Now join each point i to the point K steps on, i + K, and do it from every point: a chord from each element to its K-step successor. If K shares no common factor with N, then adding K over and over lands on every point before it returns to 0, so the single step K reaches the whole group and the chords close into one unbroken star. With N = 12 and K = 5 that is a twelve-pointed star drawn in one stroke. Any K coprime to N does the same, which is why the cyclic group of order N has exactly as many single-step generators as there are numbers below N sharing no factor with it.", kind: "star-polygon" },
          { heading: "When it splits into cosets", body: "If K and N do share a common factor d, then adding K only ever reaches points a multiple of d apart, so it generates a subgroup rather than the whole group and the figure falls into d separate polygons. Here each chord is coloured by its coset, i mod d, so the pieces come out in distinct colours: at N = 12, K = 3 there are three squares in three colours, since the gcd is 3; K = 4 gives four triangles, K = 8 also four. The number of pieces is exactly the greatest common divisor of N and K, each is one coset of the subgroup, and the subgroup itself has N / d points. Pieces times points per piece is N again, which is Lagrange's theorem drawn out: a subgroup's size always divides the group's.", kind: "star-cosets" },
        ],
      },
      {
        ...
        steps: [
          ...
          { heading: "Sweeping the multiplier", body: "At M = 2 the chords crowd along a cardioid, the heart-shaped curve. No single chord is curved, but each one is tangent to the cardioid, so together they trace its outline, the curve they envelope. Here the multiplier climbs on its own: as m sweeps upward a nephroid appears at 3 with two cusps, three cusps at 4, and in general an epicycloid with m − 1 cusps. The roots of unity sat on this same circle under addition; under multiplication the cyclic group draws a whole family of classical curves.", kind: "times-table-sweep" },
        ],
      },
    ],
  },
    ...
    pages: [
      {
        ...
        steps: [
          { heading: "A solid from shared data", body: "A cube is eight corners and twelve edges. With first-class Lists you build the eight corner positions from one expression over the index 0 to 7 (reading the index in binary picks the plus or minus on each axis), then give a second list of index pairs naming which corners each edge joins, so the edges reference the vertices rather than copying their coordinates. The corners are one source of truth: change how they are computed and every edge that uses them moves. The structure of the solid is right there as data: V = 8 vertices, E = 12 edges, and its six square faces give F = 6.", kind: "list-cube" },
          { heading: "Count V − E + F", body: "Now the six faces are filled in so you can count them. Take vertices minus edges plus faces. For the cube: 8 − 12 + 6 = 2. That combination, V − E + F, is the Euler characteristic χ. It looks like a coincidence of the cube, until you try it on something else.", kind: "list-cube-faces" },
      ...
      },
      {
        ...
        steps: [
          ...
          { heading: "An invariant you cannot deform away", body: "Flatten the same loop into the plane and add a wobble: the radius gets a bump of amplitude e, so dragging e bends the loop in and out. As long as it never crosses the centre dot, the number of times it wraps the origin does not budge. The only way to change the count is to push the curve through the centre, which is a cut rather than a deformation. That is what makes the winding number a topological invariant: it is the degree of the map sending the circle to the circle by t to w·t, a number that tells maps apart up to deformation.", kind: "winding-flat" },
          { heading: "The same number the colours wound", body: "This is the integer the domain colouring already showed. There the hue ran once around a simple zero and twice around a double zero of f(z), which is the winding number of the loop the output traces as z circles the zero. Counting how many times the colours cycle, or how many times the lifted loop climbs, is the argument principle: the winding number of the output around 0 counts the zeros enclosed (minus any poles). Plot, colour, and lift are three views of one integer. The picture here is f(z) = z², whose hue winds twice around its double zero.", kind: "dc-sq" },
        ],
      },
      {
        ...
        steps: [
          ...
          { heading: "A dimension between two and three", body: "If a shape is made of N copies of itself each scaled down by a factor s, its dimension is log N / log s, the rule that gives a line dimension 1 and a square dimension 2. Shown here at depth two, where the six half-size copies are easy to pick out: halve the scale and six copies appear, so the dimension is log 6 / log 2 ≈ 2.585, between a surface (2) and a solid (3). A non-integer dimension is what 'fractal' refers to.", kind: "sierpinski-low" },
          { heading: "Structure as data, geometry as a rule", body: "The centres are not stored anywhere. Each cell's position is derived inside one raw-geometry index template from the cell's number, read as a base-six expansion where every digit picks one of the six half-scaling directions: the recursion written as a formula. One octahedron's eight faces are then stamped at that position. Here a shallower depth is coloured by cell index, so each octahedron reads as one step of the count. One rule evaluated per cell is how the depth-six shape (46,656 octahedra) stays a tiny graph rather than a table of coordinates.", kind: "sierpinski-data" },
          { heading: "A fractal that is exactly two-dimensional", body: "Self-similar dimension need not be irrational, or even fractional. The Sierpiński tetrahedron replaces each tetrahedron with four half-size copies at its corners (N = 4 copies at scale s = 2), so its dimension is log 4 / log 2 = 2, on the nose. It is a true fractal, infinitely detailed and self-similar at every scale, that nonetheless carries the dimension of a plain surface (its faces even tile flat). Here it is drawn as a wireframe whose every edge is derived from the cell number by a base-four expansion (each digit picks one of the four half-scaling corners), the same recursion-as-formula idea as the octahedron. Different recursion, different dimension, same idea.", kind: "tut-sierp-tetra" },
        ],
      },
      {
        slug: "knots",
  ...
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        steps: [
          { heading: "An orbit of the logistic map", body: "A dynamical system is a rule applied repeatedly. The logistic map xₙ₊₁ = r·xₙ(1−xₙ) is the textbook example. Plotting an orbit as the sequence of values shows the behavior directly: at r = 3.6 the values never settle, bouncing chaotically. A raw-geometry node in recurrence mode is exactly the tool for this, with each new value computed from the last.", kind: "tut-orbit-logistic" },
