  ...
  return<div>
    ...
    {rows.map((row,i)=>(
      <div key={i} style={{display:"flex",gap:3,marginBottom:3,flexWrap:"wrap"}}>
        {row.map(t=>{const m=TYPE_META[t]||{tc:ui.uiAccent};return<button key={t} onClick={()=>onAddNode(t)} style={{...S.btnSm,flex:"1 1 22%",color:m.tc,borderColor:m.tc+"33",textAlign:"center"}}>{labelOf[t]||t}</button>;})}
      </div>
    ))}
  </div>;
}
