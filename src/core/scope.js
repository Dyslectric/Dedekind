...
function geomSignature(node, scope){
  ...
  switch(node.type){
    ...
    case "transformer": return `tr|${c}|${p.mode}|${p.domainSrc}|${p.camRes||""}|${p.inAxis0}|${p.inAxis1}|${p.inAxis2}|${p.outAxis0}|${p.outAxis1}|${p.outAxis2}|${p.outAxis3}|${p.normalize?1:0}|${resolveNum(p.arrowLen,scope,0.5)}|${p.aMin}|${p.aMax}|${p.bMin}|${p.bMax}|${p.cMin}|${p.cMax}|${p.dMin}|${p.dMax}|${resolveNum(p.res,scope,60)}|${p.cplxMode||""}|${p.colorSource||""}|${p.colorExpr||""}|${p.colorMode||""}|${p.colorShift||""}|${p.colorLo||""}|${p.colorHi||""}|${p.colorMin||""}|${p.colorMax||""}|${p.showWire!==false?1:0}|${p.wireOnly?1:0}|sh:${p.shading||""}|ul:${p.matUnlit?1:0}|${p.matColorMode||""}|${p.matColor||""}|${p.matR||""}|${p.matG||""}|${p.matB||""}|${p.matColorLo||""}|${p.matColorHi||""}|${p.matColorMin||""}|${p.matColorMax||""}|${p.matSpec||""}|${p.matEmit||""}|${p.matEmitColor||""}|uv:${p.uvScaleU||""},${p.uvScaleV||""},${p.uvOffU||""},${p.uvOffV||""},${p.uvRot||""}|ns:${p.matNormalStrength||""}|${p.__texSig||""}|${p.__fnSig||""}|fn:${p.__fnDefSig||""}|${p.__paramSig||""}|${p.__eqSig||""}|${
