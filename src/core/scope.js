...
function geomSignature(node, scope){
  ...
  switch(node.type){
    ...
    case "mesh": return `msh|${c}|${p.__dataSig||(p.data?p.data.length:0)}|${resolveNum(p.scale,scope,1)}|${p.lit!==false?1:0}|${resolveNum(p.opacity,scope,1)}|${resolveNum(p.shininess,scope,36)}|${p.flatShading?1:0}|${p.doubleSide===false?0:1}|${p.showWire?1:0}|${p.castShadow!==false?1:0}|${p.receiveShadow!==false?1:0}|${scopeSig(node,scope)}`;
